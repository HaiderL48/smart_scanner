...
import 'package:smart_scanner/features/passbook_scanner/presentation/widgets/passbook_details_widget.dart';
...
class _PassbookScannerScreenState extends State<PassbookScannerScreen> {
  ...
  Future<bool> _requestCameraPermission() async {
    ...
    if (status.isDenied || status.isPermanentlyDenied) {
      setState(() {
        _errorMessage = 'Camera permission is required to scan the Passbook.';
      });
      return false;
    }
    return true;
  ...
  Future<void> _processImage(File imageFile) async {
    ...
    try {
      ...
      final bankDetails = _passbookParser.parsePassbook(rawText);

      // Duplicate check
      if (_bankDetails != null &&
          _bankDetails!.accountNumber != null &&
          _bankDetails!.accountNumber == bankDetails.accountNumber) {
        setState(() {
          _isLoading = false;
      ...
      setState(() {
        ...
        if (bankDetails.isEmpty) {
          _errorMessage = _getErrorMessage(bankDetails);
        }
      });
    } catch (error) {
      setState(() {
  ...
  String _getErrorMessage(BankDetails details) {
    if (details.accountNumber == null && details.accountHolderName == null) {
      return 'Could not read Passbook. Make sure the Passbook is '
    ...
    if (details.accountHolderName == null) {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            if (_bankDetails != null && !_bankDetails!.isEmpty)
              PassbookDetailsWidget(bankDetails: _bankDetails),
          ],
        ),
      ),
    );
